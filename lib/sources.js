...
export async function runSource(id, domain) {
  ...
  switch (id) {
    case "crtsh": return crtsh(d);
    case "certspotter": return certspotter(d);
    case "hackertarget": return hackertarget(d);
    case "anubis": return anubis(d);
    case "otx": return otx(d);
    case "wayback": return wayback(d);
    case "urlscan": return urlscan(d);
    case "rdap": return rdap(d);
    case "doh": return doh(d);
    case "hackertarget_dns": return hackertargetDns(d);
    case "securitytrails": return securitytrails(d);
    case "github_code": return githubCode(d);
    case "virustotal": return virustotal(d);
    default: return { source: id, ok: false, error: "unknown source", data: {} };
  }
}
...
async function crtsh(domain) {
  ...
  return { source: "crtsh", ok: r.ok, error: r.ok ? null : r.text.slice(0, 160), data: { subdomains: uniq(names).slice(0, 2000) } };
}
...
async function otx(domain) {
  ...
  return {
    ...
    data: {
      ...
      whois: gen.json?.whois || null
    }
  };
}

async function wayback(domain) {
  const r = await fetchJson(`https://web.archive.org/cdx/search/cdx?url=*.${encodeURIComponent(domain)}/*&output=json&fl=original&collapse=urlkey&limit=200`);
  ...
  const interesting = urls.filter((u) => /admin|api|graphql|swagger|debug|backup|\.git|\.env|token|config|internal/i.test(u)).slice(0, 80);
  return { source: "wayback", ok: r.ok, data: { urls: urls.slice(0, 200), paths, interesting } };
}
...
async function urlscan(domain) {
  ...
  const ips = results.map((x) => x.page?.ip).filter(Boolean);
  return { source: "urlscan", ok: r.ok, error: r.ok ? null : r.text.slice(0, 160), data: { subdomains: uniq(subdomains), ips: uniq(ips), scans: results.slice(0, 20).map((x) => ({ url: x.page?.url, ip: x.page?.ip, server: x.page?.server })) } };
}
...
async function rdap(domain) {
  ...
  const walk = (obj) => {
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (typeof v === "string" && /@/.test(v) && /email/i.test(k + JSON.stringify(obj.roles || ""))) emails.push(v.toLowerCase());
      else walk(v);
    }
  };
  walk(r.json?.entities);
  return { source: "rdap", ok: r.ok, data: { registrar: r.json?.registrar || r.json?.ldhName, nameservers, emails: uniq(emails), status: r.json?.status || [], rawOk: r.ok } };
}
...
async function securitytrails(domain) {
  ...
  return { source: "securitytrails", ok: r.ok, error: r.ok ? null : r.text.slice(0, 160), data: { subdomains: subs.slice(0, 2000) } };
}
...
async function githubCode(domain) {
  ...
  return {
    source: "github_code",
    ok: r.status === 200,
    error: r.status === 200 ? null : `GitHub ${r.status} (auth raises rate limit)`,
    data: {
...
async function virustotal(domain) {
  ...
  return { source: "virustotal", ok: r.ok, data: { stats: r.json?.data?.attributes?.last_analysis_stats || {}, categories: r.json?.data?.attributes?.categories || {} } };
}
